...
func createAndDeleteStream(errChl chan error) {
	...
	for {
		...
		case err := <-errChl:
			ticker.Stop()
			fmt.Printf("Got an error from MA, this probram will be stopped. Error: %v", err.Error())
...
func collectLogs() {
	...
	defer fmt.Printf("%v","Finish to collect log.")
	...
	for epIPAddr, epObjRef := range endpointsMap {
		...
		if len(maStatus.StreamID) != 0 {
			...
			if anotherEndpoint, found := streamMap[maStatus.StreamID]; found {
				...
				errChl <- fmt.Errorf("%v","Found multiple MA hold the same stream!")
